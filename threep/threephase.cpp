...
int main(int argc,char* argv[]) {
	...
	if(!files) {
		...
		Mat blank = Mat::zeros(h,w,CV_8UC3);
		...
		createPhaseImg(p3,p2,p1,frq,gamma,vertical);
	...
	*/
	...
	while(1) {
		...
		if(changed) {
			changed = false;
			ph.setScale((float)(sk-128)/4.,(float)(sc-128)/4.,(float)(nthr)/512.,(float)(contr-128));
			if(noiseChanged) {
				ph.phaseWrap();	
				ph.phaseUnwrap();
				noiseChanged = false;
...
_OutputArray::~_OutputArray(void) {}
_InputArray::~_InputArray(void) {}
